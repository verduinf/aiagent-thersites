...
## Overview
This document specifies the strict security boundaries and tool execution rules for **Thersites** ("The Intern"). All tool calls issued by Thersites are programmatically intercepted and verified by **The Bouncer** (`bouncer.py`) before touching disk or network.
...
* **Path Enforcement**:
  ...
  * Any attempt to access, write, read, or delete files outside this directory (e.g. `../../etc/passwd` or system files) triggers an immediate security exception by The Bouncer:
...
* **Domain Validation**:
  ...
  * If Thersites requests any domain other than `nu.nl` (e.g. `google.com`, `malicious-site.com`), The Bouncer intercepts the call and returns:
    > `[ERROR: Unauthorized domain. URL must be in whitelist: ['nu.nl']]`

---

## 🛡️ 3. Bouncer Enforcement Contract

```
[ Thersites (Qwen3-9B) Action JSON ] ──► [ THE BOUNCER (bouncer.py) ]
                                                   │
                                     ┌─────────────┴─────────────┐
                                     ▼                           ▼
