...
* **AFFIRMATIVE INSTRUCTIONS ONLY**: Never instruct a local model using negative constraints, prohibitions, or forbidden tokens (e.g., avoid "Do not mention X", "Skip the `<snapshot>` block", "No internal essays", "Never output plain text", "Never write outside sandbox").
...
* **MANDATORY PATTERN**: Frame every directive as a direct, affirmative action describing *what to do* rather than what to avoid:
  - *Intern JSON Anti-Pattern*: "Do not write plain text or markdown code fences outside the JSON object."
  - *Affirmative Pattern*: "Package your entire turn output strictly inside the single valid JSON contract schema with `thought`, `content`, and `actions` fields."
  - *Sandbox Anti-Pattern*: "Do not write files outside `sandbox/`."
  - *Affirmative Pattern*: "Direct all file creations and edits strictly inside `sandbox/`."
  - *Anti-Pattern*: "Avoid lengthy internal essays or multi-paragraph dissertations."
  - *Affirmative Pattern*: "Restrict your response to compact, actionable turns (3 to 5 sentences maximum in `content`)."
* **PROGRAMMATIC BOUNDARIES OVER PROMPT WARNINGS**: If tokens, paths, structures, or formats must never be breached, enforce them programmatically with robust backend interceptors, sanitizers, and The Warden (`core/warden.py`) rather than relying on prompt-level negative commands.
